...
struct MatchAddress : public Worker
{
  ...
  MatchAddress(Rcpp::CharacterVector input, const Rcpp::CharacterVector target, Rcpp::NumericMatrix output)
...
library(stringi)
set.seed(2)
input <- stringi::stri_rand_strings(100, 5)
target <- stringi::stri_rand_strings(550000, 5)
target[11] <- input[1]

result <- parallelMatrixSqrt(input, target)

system.time(parallelMatrixSqrt(input, target))
